...
def scrape_montana_ucc(limit=50):
    """
    Playwright scraper for Montana UCC filings.
    Target: biz.sosmt.gov
    """
    conn = setup_db()
    leads_inserted = 0

    print("Launching Playwright for Montana UCC (biz.sosmt.gov)...")
    with sync_playwright() as p:
        ...
        try:
            # Navigate to the Montana SOS UCC Search page
            page.goto("https://biz.sosmt.gov/search/ucc", timeout=60000)
            page.wait_for_load_state("networkidle")
            print("Successfully loaded Montana SOS portal.")

            # Core DOM execution structure (requires maintenance if site layout changes)
            # MT economy: agriculture/ranching, logging, mining, construction,
            # trucking, healthcare, credit unions, energy, tourism
            lender_keywords = [
                # Original
                ...
                # MT agriculture & natural resources
                "Ranch", "Grain", "Livestock", "Agriculture", "Irrigation",
            ...
            print(f"Error navigating Montana SOS: {e}")
        finally:
            conn.commit()
            conn.close()
            browser.close()
            
    print(f"Montana Scraper Complete. Inserted {leads_inserted} new leads into tomcat_capex.db.")
...
if __name__ == "__main__":
    ...
    scrape_montana_ucc(limit=args.limit)
